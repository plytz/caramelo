...
func buildProvisionedImage(ctx context.Context) (string, error) {
	...
	fmt.Fprintf(os.Stderr, "itest: provisioning %s (fleet setup, this takes minutes)\n", tag)
	start := time.Now()
	...
	if jsonErr := json.Unmarshal([]byte(strings.TrimSpace(res.Stdout)), &report); jsonErr != nil {
		return "", fmt.Errorf("provision: fleet setup exit %d, stdout is not a report: %w\nstdout:\n%s\nstderr:\n%s",
			res.ExitCode, jsonErr, res.Stdout, res.Stderr)
	}
	if res.ExitCode != 0 || report.Failed != 0 {
		return "", fmt.Errorf("provision: fleet setup exit %d, %d step(s) failed\n%s",
