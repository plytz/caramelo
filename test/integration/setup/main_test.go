...
func start() (err error) {
	...
	if firstRunErr == nil {
		if err := itest.WaitForCaramelod(ctx, machine); err != nil {
			firstRunErr = fmt.Errorf("server setup finished but caramelod never came up: %w", err)
...
func runSetup(ctx context.Context, m *itest.Machine, bin string) (setuppkg.Report, string, error) {
	...
	if jsonErr := json.Unmarshal([]byte(strings.TrimSpace(res.Stdout)), &report); jsonErr != nil {
		return report, raw, fmt.Errorf("server setup: exit %d, stdout is not a setup.Report: %w", res.ExitCode, jsonErr)
	}
	if res.ExitCode != 0 {
		return report, raw, fmt.Errorf("server setup: exit %d", res.ExitCode)
	}
