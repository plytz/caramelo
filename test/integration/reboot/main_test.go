...
const suiteName = "reboot"

var errSuiteAborted = errors.New("the reboot suite aborted while starting")
...
func (b *suiteTB) TempDir() string {
	dir, err := os.MkdirTemp("", "caramelo-reboot-suite")
	if err != nil {
...
func runSuite(m *testing.M) int {
	...
	startErr = start()
	if startErr != nil {
		fmt.Fprintf(os.Stderr, "reboot suite: %v\n", startErr)
	}
	return m.Run()
}
...
func start() (err error) {
	defer func() {
		if r := recover(); r != nil {
			err = fmt.Errorf("starting the reboot suite: %v", r)
		}
	}()

	if _, binErr := itest.BinaryPathOr(); binErr != nil {
		skipReason = binErr.Error()
		fmt.Fprintf(os.Stderr, "reboot suite: %v; tests will skip\n", binErr)
		return nil
	}
	...
	res, err := box.Run(ctx, "systemctl is-active "+csetup.EdgeSocketUnit)
	if err != nil {
		return err
	}
	if strings.TrimSpace(res.Stdout) != "active" {
		fmt.Fprintf(os.Stderr, "reboot suite: this machine has no edge (%s is %q); the edge checks will skip\n",
			csetup.EdgeSocketUnit, strings.TrimSpace(res.Stdout))
		return nil
	}
...
func refreshEdgeTrust() error {
	...
	defer cancel()
	ready := "curl -sk --max-time 5 " + itest.PebbleDirectory + " >/dev/null"
	if err := box.WaitFor(ctx, ready, 2*time.Second); err != nil {
		return fmt.Errorf("pebble never came back after the power cycle: %w", err)
	}
	if err := pebble.Refresh(box); err != nil {
		return fmt.Errorf("re-read pebble's issuing root after the power cycle: %w", err)
	}
...
func restartEdgeOn(ctx context.Context, m *itest.Machine) error {
	res, err := m.RunAsRoot(ctx, "systemctl restart "+csetup.EdgeServiceUnit)
	if err != nil {
		return err
	}
	if res.ExitCode != 0 {
...
func waitForEdgeSocket(ctx context.Context, m *itest.Machine, budget time.Duration) error {
	...
	for {
		if r, err := m.RunAsRoot(ctx, "test -S "+edgeControlSocket); err == nil && r.ExitCode == 0 {
			return nil
		}
