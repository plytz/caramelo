...
func TestRunShipsAndRunsSetupWithSudo(t *testing.T) {
	...
	sh := &fakeShell{t: t, answers: []answer{
		...
		{prefix: "sudo -n /tmp/caramelo-setup.abc123/caramelo fleet setup", stdout: reportJSON(t, want), stderr: "[changed] user\n"},
		{prefix: "rm -rf", code: 0},
	}}
...
func TestRunAsRootWithLocalKeys(t *testing.T) {
	...
	sh := &fakeShell{t: t, answers: []answer{
		...
		{prefix: "/tmp/caramelo-setup.root1/caramelo fleet setup", stdout: reportJSON(t, setup.Report{RunID: "r2"})},
		{prefix: "rm -rf"},
	}}
...
func TestRunFailedStepIsAReportNotAnError(t *testing.T) {
	...
	sh := &fakeShell{t: t, answers: []answer{
		...
		{prefix: "/tmp/caramelo-setup.y/caramelo fleet setup", stdout: reportJSON(t, rep), code: 1},
		{prefix: "rm -rf"},
	}}
...
func TestRunAdmitsAPeer(t *testing.T) {
	...
	sh := &fakeShell{t: t, answers: []answer{
		{prefix: "uname -sm", stdout: probeAnswer(PrivilegeRoot, true)},
		{prefix: "d=$(mktemp", stdout: "/tmp/caramelo-setup.abc123\n"},
		{prefix: "/tmp/caramelo-setup.abc123/caramelo fleet setup", stdout: reportJSON(t, setup.Report{RunID: "r1"})},
		{prefix: "rm -rf", code: 0},
	}}
...
func TestRunRefusesAnUnusablePeer(t *testing.T) {
	...
	for _, line := range sh.ran {
		if strings.Contains(line, "fleet setup") {
...
func TestRunShipsTheBinaryForTheTarget(t *testing.T) {
	...
	sh := &fakeShell{t: t, answers: []answer{
		...
		{prefix: "/tmp/caramelo-setup.abc/caramelo fleet setup", stdout: reportJSON(t, want)},
		{prefix: "rm -rf", code: 0},
	}}
	out, err := Run(context.Background(), sh, Options{})
	if err != nil {
