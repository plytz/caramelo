...
func TestRemovePeerRefusesTheSessionsOwnIdentity(t *testing.T) {
	...
	ctx := sshapi.WithSession(context.Background(), api.Session{
		Transport: "tunnel", Identity: "laptop",
	})

	err := d.RemovePeer(ctx, "laptop", false)
	if err == nil {
		t.Fatal("removing this session's own peer was allowed")
	}
	for _, want := range []string{"laptop", "this session arrived as", "--force"} {
...
func TestRemovePeerAllowsEveryOtherCase(t *testing.T) {
	d := &sshapi.Daemon{}
	own := sshapi.WithSession(context.Background(), api.Session{Transport: "tunnel", Identity: "laptop"})
	...
	for _, c := range []struct {
		...
		{"its own, forced", own, "laptop", true},
		{"the local socket, which has no identity", context.Background(), "laptop", false},
	} {
